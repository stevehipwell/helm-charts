...
| `fullnameOverride`                        | Override the full name of the chart.                                                                                            | `null`                               |
...
| `serviceAccount.name`                     | Service account to be used. If not set and `serviceAccount.create` is `true`, a name is generated using the full name template. | `null`                               |
...
| `service.additionalPorts`                 | Additional ports exposed by the service and used by repository connectors.                                                      | `null`                               |
...
| `env`                                     | Environment variables for the _nexus3_ container.                                                                               | `null`                               |
...
| `config.rootPassword.secret`              | Secret to update the root password with (must also have `config.rootPassword.key` set).                                         | `null`                               |
| `config.rootPassword.key`                 | Key on the secret set in `config.rootPassword.secret`.                                                                          | `null`                               |
...
| `config.ldap.name`                        | Unique name for the LDAP configuration.                                                                                         | `null`                               |
...
| `config.ldap.host`                        | LDAP host.                                                                                                                      | `null`                               |
| `config.ldap.port`                        | LDAP port.                                                                                                                      | `636`                                |
| `config.ldap.authScheme`                  | LDAP authentication schema.                                                                                                     | `simple`                             |
| `config.ldap.authUsername`                | Username or DN (Distinguished Name) of an LDAP user, used to connect to the LDAP server.                                        | `null`                               |
| `config.ldap.authPassword.secret`         | Secret containing the password to connect to the LDAP server.                                                                   | `null`                               |
| `config.ldap.authPassword.key`            | The key on the secret containing the password to connect to the LDAP server.                                                    | `null`                               |
| `config.ldap.authRealm`                   | LDAP authentication realm.                                                                                                      | `null`                               |
| `config.ldap.searchBase`                  | LDAP search base.                                                                                                               | `null`                               |
| `config.ldap.userBaseDn`                  | LDAP user base, relative to the search base.                                                                                    | `null`                               |
...
| `config.ldap.userLdapFilter`              | LDAP user filter.                                                                                                               | `null`                               |
...
| `config.ldap.userPasswordAttribute`       | LDAP user password attribute.                                                                                                   | `null`                               |
...
| `config.ldap.groupBaseDn`                 | LDAP group base, required if `groupType` is `static`.                                                                           | `null`                               |
...
| `config.ldap.groupObjectClass`            | LDAP group object class, required if `groupType` is `static`.                                                                   | `null`                               |
| `config.ldap.groupIdAttribute`            | LDAP group id attribute, required if `groupType` is `static`.                                                                   | `null`                               |
| `config.ldap.groupMemberAttribute`        | LDAP group member attribute, required if `groupType` is `static`.                                                               | `null`                               |
| `config.ldap.groupMemberFormat`           | LDAP group member format, required if `groupType` is `static`.                                                                  | `null`                               |
