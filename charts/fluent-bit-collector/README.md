...
| hostVolumes | list | `[{"hostPath":{"path":"/etc/machine-id","type":"File"},"mountPath":"/etc/machine-id","name":"machine-id"},{"hostPath":{"path":"/var/log"},"mountPath":"/var/log","name":"logs"}]` | Host volumes to read-only mount to the default container. |
